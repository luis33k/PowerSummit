...
def get_top_kpis(df: pd.DataFrame) -> dict:
    ...
    if not df.empty:
        ...
        ctl_col = next((col for col in df.columns if 'CTL (42d EWMA)' in col), None)
        atl_col = next((col for col in df.columns if 'ATL (7d EWMA)' in col), None)
        ...
        kpis = {
            ...
            'Avg Sleep 7d': df.get(f'{sleep_col} (7d Avg)', df[sleep_col]).iloc[-1] if sleep_col else 0.0
        }
